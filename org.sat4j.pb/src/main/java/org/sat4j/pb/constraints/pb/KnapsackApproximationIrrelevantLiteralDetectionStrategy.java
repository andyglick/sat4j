...
public class KnapsackApproximationIrrelevantLiteralDetectionStrategy
    ...
    public boolean dependsOn(int nVars, IVecInt literals,
        ...
        for (int i = 0; i < literals.size(); i++) {
            if (i != literalIndex && !coefficients[i].equals(degree)) {
                max = max.max(coefficients[i]);
                size++;
            }
        }
        if (size == 0) {
            return coefficients[literalIndex].compareTo(degree) >= 0;
        }
        // Reducing the sizes.
